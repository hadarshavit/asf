...
def get_data():
    ...
    for i in range(n_instances):
        ...
        times = [max(5, t) for t in [algo1_time, algo2_time, algo3_time, algo4_time, algo5_time]]
        performance_data.append(times)
    
    features = pd.DataFrame(
        feature_data, 
        columns=["complexity", "size", "density", "structure", "noise"]
    )
    
    performance = pd.DataFrame(
        performance_data, 
        columns=["algo1", "algo2", "algo3", "algo4", "algo5"]
    )
...
if __name__ == "__main__":
    ...
    print(f"Dataset: {len(features)} instances ({train_size} train, {len(test_features)} test)")
    ...
    for alg, time in asap_config.items():
        solvable_by_alg[alg] = set(test_performance[test_performance[alg] <= time].index)
    ...
    for alg, time in asap_config.items():
        ...
        print(f"  {alg}: {time:.1f}s (solves {solved}/{len(test_performance)} = {solve_rate:.0f}%, unique: {unique_count})")
    ...
    for i, (instance_id, prediction) in enumerate(list(predictions.items())[:3]):
        main_alg = prediction[0][0] if isinstance(prediction, list) and len(prediction) > 0 else str(prediction)
        best_actual = test_performance.loc[instance_id].idxmin()
        best_time = test_performance.loc[instance_id].min()
        
        print(f"  {instance_id}: [{preschedule_str}] → {main_alg} (best: {best_actual}:{best_time:.0f}s)")